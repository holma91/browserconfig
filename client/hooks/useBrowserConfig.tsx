...
const fetcher = async (domain: string) => {
  ...
  let preferenceKeys: ('popUps' | 'darkMode')[] = ['popUps', 'darkMode'];
  for (const preferenceKey of preferenceKeys) {
    if (config.preferences[preferenceKey].allow) {
      processedConfig.preferences[preferenceKey] =
        !config.preferences[preferenceKey].exceptions.includes(domain);
    } else {
      processedConfig.preferences[preferenceKey] =
        config.preferences[preferenceKey].exceptions.includes(domain);
    }
